// 11/22 aj_ra_input_daily (with total spending, category dropdown, and undefined purchase buttons removed)
...
function InputDailySpending() {
    ...
    //CHANGE 1: make arePurchasesStored boolean correspond to whether inputDailyObj has purchases, instead of the useState var
            ...
            <div>
                {isAddMode && (
                    purchases.map((purchase, index) => (
                        <div key={index}>
                            {/*{(!arePurchasesStored) && (*/}
                            {/* CHANGE 2: remove !inputDailyObj condition altogether - if in add mode,
                            the purchases will display underneath*/}
                            {/*{(!inputDailyObj) && (*/}
                                <div>
...
//             setNoSpendingMessage(noSpendingMessage);
//         } else {
//             setNoSpendingMessage("Today's purchases:");
//             setIsSubmitted(true);
//         }
//         setShowPurchaseFields(false);
//         setPurchases(purchases.slice().reverse());
...
//             setEditIndex(null); // Reset editIndex
//             setIsEditing(false); // Exit "Edit" mode
//             // Reset input fields and any error noSpendingMessages
//             setPurchasedItem('');
//             setPurchaseAmount('');
//             setSelectedCategory('');
//             setInputPurchaseError('');
...
//                         </div>
//                         {amountError && <p className="error-noSpendingMessage2">{amountError}</p>}
//                         {inputPurchaseError && <p className="error-noSpendingMessage3">{inputPurchaseError}</p>}
//                         <button
//                             className="add-button2"
